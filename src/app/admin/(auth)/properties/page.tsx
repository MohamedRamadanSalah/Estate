...
import { Plus, ExternalLink, Edit3, ImageIcon, ChevronLeft } from 'lucide-react';
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-navy">إدارة الوحدات</h1>
          <p className="text-gray-400 text-sm mt-1">{properties.length} وحدة عقارية</p>
        </div>
        <Link
          href="/admin/properties/new"
          className="flex items-center justify-center gap-2 px-5 py-2.5 bg-gold text-navy font-semibold rounded-xl hover:bg-copper transition-colors shadow-md hover:shadow-lg text-sm sm:text-base"
        >
      ...
      <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        {properties.length === 0 ? (
          <div className="p-8 sm:p-12 text-center">
            ...
            <p className="text-gray-500 font-medium mb-2 text-sm">لا توجد وحدات</p>
            <p className="text-gray-400 text-xs sm:text-sm">اضغط &quot;إضافة وحدة جديدة&quot; للبدء</p>
          </div>
        ) : (
          <>
            {/* Mobile card view */}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-mono text-[10px] bg-navy/5 text-navy px-1.5 py-0.5 rounded">{p.unitCode}</span>
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold ${statusBadge(p.status)}`}>
                          {p.status}
                        </span>
                      </div>
                      <p className="font-semibold text-navy text-sm truncate">{p.projectName}</p>
                      <p className="text-xs text-gray-400 truncate">{p.governorate?.nameAr} - {p.city?.nameAr}</p>
                      <p className="text-sm font-bold text-navy mt-1">{Number(p.totalPrice).toLocaleString('ar-EG')} <span className="text-[10px] font-normal text-gray-400">ج.م</span></p>
                    </div>
                  </div>
                        ...
                        <>
                          <div className="flex -space-x-1.5 rtl:space-x-reverse">
                            {p.images.slice(0, 4).map((img, i) => (
                              <div key={i} className="w-7 h-7 rounded-lg overflow-hidden border-2 border-white shadow-sm">
                                <Image src={img} alt="" width={28} height={28} className="w-full h-full object-cover" unoptimized />
                              </div>
                            ))}
                          </div>
                          {p.images.length > 4 && (
                            <span className="text-[10px] text-gray-400 mr-1">+{p.images.length - 4}</span>
                          )}
                        </>
                      ) : (
                        <span className="text-[10px] text-gray-400">لا صور</span>
                      )}
                    </div>
